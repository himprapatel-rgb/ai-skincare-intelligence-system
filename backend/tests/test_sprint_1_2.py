...
class TestOnboardingFlow:
    ...
    def test_onboarding_step_1_goals(self, auth_headers):
        ...
        data = {"step": 1, "goals": ["acne_reduction", "anti_aging", "hydration"]}
        response = client.post(
            "/api/v1/onboarding/step", json=data, headers=auth_headers
        )
    ...
    def test_onboarding_step_2_concerns(self, auth_headers):
        ...
        data = {"step": 2, "concerns": ["acne", "redness", "dark_spots"]}
        response = client.post(
            "/api/v1/onboarding/step", json=data, headers=auth_headers
        )
        assert response.status_code == 200
...
class TestProfileManagement:
    ...
    def test_update_profile_basic(self, auth_headers):
        ...
        data = {"name": "Updated Name", "phone": "+353-123-456-7890"}
    ...
    def test_update_profile_settings(self, auth_headers):
        ...
        response = client.patch(
            "/api/v1/profile/settings", json=data, headers=auth_headers
        )
        assert response.status_code == 200
...
class TestConsentPrivacy:
    ...
    def test_accept_consent(self, auth_headers):
        ...
        response = client.post(
            "/api/v1/consent/accept", json=data, headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_reject_consent(self, auth_headers):
        ...
        data = {"policy_version": "1.0", "accepted": False}
        response = client.post(
            "/api/v1/consent/reject", json=data, headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_consent_version_tracking(self, auth_headers):
        ...
        client.post(
            ...
            json={"policy_version": "1.0", "accepted": True},
