# AI Skincare Intelligence System -- SRS V5.3 - External ML Model Integration
...
- ### V5.3 Change Summary (CR-ML-003)

**Context:** Implementing external pre-trained ML models for MVP while deferring dataset-based training to post-MVP phases.

**Key Changes:**
- Added FR6A-FR6F: External ML model storage, runtime loading, model-agnostic architecture
- Added NFR21-NFR25: External storage requirements, secure model access, CI/CD compatibility
- Added DR4A-DR4B: User data storage for future ML training
- Updated DR1: Open-source dataset training marked as Deferred/Post-MVP
- Clarified MVP uses pre-trained, inference-only models
- Models stored on Railway volumes or external object storage (not GitHub)
- Support for models >100 MB without breaking GitHub limits
...
Current AI skincare solutions are fragmented. Users typically need separate tools for skin 

### 1.5 AI Inference-Only MVP Strategy (V5.3)

For MVP, the AI Skincare Intelligence System uses **pre-trained, inference-only ML models** for face scanning and skin condition analysis instead of training on open-source datasets during early phases. Models are treated as external, replaceable artifacts loaded at runtime from persistent storage attached to the backend (Railway volume) or from approved external object storage (e.g., Google Drive, cloud object storage) with local caching.

All skin analysis, scores, and confidence values are generated by these pre-trained models, while the platform **stores user images, analysis outputs, and metadata** to support future fine-tuning, custom model training, and personalization in post-MVP phases. Open-source dataset-based training remains in scope but is explicitly deferred to later releases.scans, routines, ingredient safety, counterfeit checks, and education. Existing apps often ignore long-term evolution, environmental impact, ingredient interactions, and counterfeit risk, and frequently underperform across diverse skin tones.
...
- **FR9B:** When confidence is below threshold, mark outputs as uncertain and guide to resc
-
- - **FR6A (NEW - V5.3):** The system SHALL perform skin analysis using one or more **external pre-trained ML models** (inference-only) loaded at runtime from: (a) a Railway persistent volume mounted into the backend service; or (b) an external object storage location (e.g., Google Drive, cloud object storage) downloaded and cached on local disk.

- **FR6B (NEW - V5.3):** The skin analysis engine SHALL be **model-agnostic**, allowing models to be swapped or upgraded (e.g., different architectures, vendors, or fine-tuned variants) without requiring any change to frontend contracts or API response schemas.

- **FR6C (NEW - V5.3):** The backend API SHALL expose a **stable analysis response schema** (concerns, scores, confidence values) independent of the underlying model implementation, covering at minimum: acne/pimples, rash, redness, pigmentation, wrinkles, uneven tone.

- **FR6D (NEW - V5.3):** The system SHALL support **model versioning**, tagging each inference result with `model_family` and `model_version` to enable auditability and future comparison.

- **FR6E (NEW - V5.3):** The backend SHALL implement a **non-blocking model loading mechanism**, ensuring that: (a) model loading happens at application startup or on first request using a cached loader; and (b) concurrent requests either reuse the loaded model or receive an appropriate temporary error without blocking the event loop.

- **FR6F (NEW - V5.3):** The system SHALL store for each scan: original image (or a privacy-preserving processed variant, per user consent), structured analysis results, model metadata (model id, version), relevant context (lighting flags, device type), to support future ML training and evaluation.

- **FR9C (UPDATED - V5.3):** Personalized model tuning SHALL be performed only in **post-MVP phases**, using stored user data (images + results + metadata) and, when introduced, open-source datasets under DR1/DR1A. For MVP, no on-platform training or fine-tuning is executed in production.an or consult professional
...
- **DR1:** Use dermatology datasets (acne, wrinkles, pigmentation, text(UPDATED - V5.3):** Use dermatology datasets (acne, wrinkles, pigmentation, texture, face segmentation) for ML training in **post-MVP phases only**. For MVP, models are assumed to be pre-trained outside the production system and delivered as artifacts.for initial ML training
...
- - **DR1B (NEW - V5.3):** The SRS explicitly marks dataset-based model training and re-training as **Deferred / Post-MVP** activities. No training jobs run in the production environment during MVP.
...
- - **DR4A (NEW - V5.3):** The system SHALL persist user images, analysis results, and metadata in the internal database/storage (subject to consent and data minimization) in a format suitable for future supervised/weakly-supervised training and evaluation.

- **DR4B (NEW - V5.3):** The data schema SHALL maintain links between scans, model versions, and subsequent outcomes to enable later fine-tuning and personalization experiments.
...
- **NFR20:** Testability - All requirements stated in measurable, objectively verifiable terms
-
- - **NFR21 - External Model Storage (NEW - V5.3):** ML models larger than 100 MB SHALL NOT be stored in the GitHub repository. Instead, they MUST be stored either on: (a) Railway persistent storage (volume) mounted into the backend service; or (b) approved external object storage (e.g., Google Drive, cloud object storage) with runtime download and local caching.

- **NFR22 - Runtime Model Loading (NEW - V5.3):** The backend SHALL load ML models using a **lazy or startup loading strategy** that avoids per-request loading and ensures predictable latency (target: initial load may be slower; subsequent inferences must meet NFR1 latency).

- **NFR23 - Secure Model Access (NEW - V5.3):** Access to external model storage SHALL: (a) use HTTPS or equivalent encrypted transport; (b) rely on secrets managed via Railway/GitHub (API keys, signed URLs); and (c) avoid embedding credentials in client-facing code or Git history.

- **NFR24 - Model Swap Safety (NEW - V5.3):** Swapping the underlying model (upgrading or changing provider) MUST NOT break: API schemas, existing frontend integrations, or SRS traceability; any impact MUST be documented via an ADR and Change Log entry.

- **NFR25 - CI/CD Compatibility (NEW - V5.3):** CI pipelines SHALL run without requiring the full production model artifact (e.g., by using a lightweight stub model, feature flags, or conditional loading), so that pipeline runtimes and storage remain manageable.for validation
